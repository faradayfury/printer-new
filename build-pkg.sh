...
#
# This packages the rastertoxqx CUPS raster filter, PPD, firmware,
# and firmware auto-upload daemon into a distributable .pkg.
#
# No Ghostscript — uses macOS's built-in cgpdftoraster.
#
# Prerequisites:
#   - rastertoxqx compiled (will compile if missing)
#   - foo2zjs/sihpP1005.dl firmware file (will generate if missing)
#
...
if [ ! -f "$SCRIPT_DIR/rastertoxqx" ]; then
...
# Validate
[ -x "$SCRIPT_DIR/rastertoxqx" ] || die "rastertoxqx not found"
[ -f "$SCRIPT_DIR/HP-LaserJet_P1007.ppd" ] || die "PPD not found"
[ -f "$FOO2ZJS_DIR/sihpP1005.dl" ] || die "Firmware not found"
...
log "Copying rastertoxqx filter..."
cp "$SCRIPT_DIR/rastertoxqx" "$PKG_ROOT/usr/libexec/cups/filter/rastertoxqx"
...
cp "$FOO2ZJS_DIR/sihpP1005.dl" "$PKG_ROOT/usr/local/share/foo2xqx/firmware/sihpP1005.dl"
...
# ── Step 4: Set permissions & codesign ──────────────────────────────────────
...
log "Codesigning rastertoxqx..."
codesign --force --sign - "$PKG_ROOT/usr/libexec/cups/filter/rastertoxqx"
...
# Restart CUPS to pick up new filter
...
<p>The driver uses macOS's built-in PDF rasterizer with the open-source <strong>foo2xqx</strong> XQX encoder.</p>
<div class="note">
...
<ol>
    ...
    <li><strong>Add the printer</strong> in System Settings &gt; Printers &amp; Scanners:
        <br>Click <strong>+</strong>, select <strong>HP LaserJet P1007</strong> from USB, and choose <strong>"HP LaserJet P1007"</strong> as the driver.</li>
